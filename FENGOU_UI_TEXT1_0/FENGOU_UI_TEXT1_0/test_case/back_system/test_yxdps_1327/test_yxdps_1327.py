...
class TestYxdps1327:
    ...
    def test_case_001(self , init_driver):
        driver = init_driver
        Frist_menu1327.Frist_menu1327(self,driver) # 冗余代码 封装
        time.sleep(1)
        js = "window.scrollBy(0,800)" # 向下滑动1000个像素
        driver.execute_script(js)
        actual_result = common_config.ff(driver).to_text("//*[@id='J-ajaxGetBusinessCooperationPk']") # 获取所有文本
        expected_result = "1月新增有效合作商家数挑战赛" # 预期结果文本
    ...
    def test_case_002(self,init_driver):
        driver = init_driver
        Frist_menu1327.Frist_menu1327(self, driver)
        time.sleep(1)
        js = "window.scrollBy(0,800)"  # 向下滑动1000个像素
        driver.execute_script(js)
        common_config.ff(driver).click("//*[@class='J-showBusinessCooperation']")
        pass